...
def get_fcn4s_symbol(numclass=21, workspace_default=1024):
    data = mx.symbol.Variable(name="data")
    pool3, pool2 = vgg16_pool3(data, workspace_default)
    pool4 = vgg16_pool4(pool3, workspace_default)
    score = vgg16_score(pool4, numclass, workspace_default)
    ...
    # score 8X
    score8 = mx.symbol.Deconvolution(data=score_final, kernel=(4, 4), stride=(2, 2), num_filter=numclass,
                                     adj=(1, 1), workspace=workspace_default, name="score8")  # 8X
    score_pool2 = mx.symbol.Convolution(data=pool2, kernel=(1, 1), num_filter=numclass,
                                        workspace=workspace_default, name="score_pool2")
    score_pool8c = mx.symbol.Crop(*[score_pool2, score8], offset=offset()["score_pool3c"], name="score_pool8c")
    score_final = score8 + score_pool8c

    softmax = fcnxs_score(score_final, data, offset()["fcn8s_upscore"], (8, 8), (4, 4), numclass, workspace_default)
    return softmax
